...
function Bootstrap3ElementModifierFn($log) {
  ...
  var reset = function (el) {
      ...
      el.removeClass('has-success is-invalid has-feedback');
    },
    ...
    makeInvalid = function (el, errorMsg) {
      var frmGroupEl = findFormGroupElement(el),
        helpTextEl = angular.element('<span class="help-block is-invalid error-msg">' + errorMsg + '</span>'),
        inputGroupEl;

      if (frmGroupEl) {
        reset(frmGroupEl);
        inputGroupEl = findInputGroupElement(frmGroupEl[0]);
        frmGroupEl.addClass('is-invalid ' + (inputGroupEl.length > 0 || addValidationStateIcons === false ? '' : 'has-feedback'));
